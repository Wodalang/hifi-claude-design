...
[What it does](#what-it-does) · [How it works](#core-mechanics) · [vs. Claude Design](#vs-claude-design)

> 📖 **Note for English readers**: the skill's agent prompts (`SKILL.md`, `references/*.md`) are in Chinese but the agent is bilingual — works fine with English tasks. The demos below are the English parallel versions; the Chinese ones are in the default-named files (see the Chinese [README.md](README.md)).
...
The Core Asset Protocol's philosophy was inspired by system prompts Anthropic wrote for Claude Design. That prompt hammers home a single idea — **great hi-fi design doesn't start from a blank page, it grows from existing design context**. That one principle is the difference between a 65-point design and a 90-point design.

Positioning differences:

| | Claude Design | hifi-claude-design |
|---|---|---|
...
Claude Design is a **better graphics tool**. hifi-claude-design makes **the graphics-tool layer disappear**. Two paths, different audiences.
