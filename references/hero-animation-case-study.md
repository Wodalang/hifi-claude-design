...
> 从 hifi-claude-design hero 动画 v9（25 秒，8 场景）里提炼出的**一种可复用的视觉编排结构**。
...
Gallery 阶段左上角有个 `hifi-claude-design` 小标识，16% opacity 色值，12px 字号，宽字距。
...
- 源 HTML（v6 + 音频集成版）：见 demos 目录下的 hero animation 源文件
